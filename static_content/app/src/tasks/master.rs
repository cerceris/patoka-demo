...
use patoka::{
    ...
    worker::{
        ...
        tracker::{self, TaskUpdate, TaskSubscription},
...
impl MasterClient {
    ...
    fn process_task_result(
        ...
        debug!(self.log, "List of countries: {:?}", res.countries);

        let connector = processor::start();
        let task_tracker = tracker::start();

        // Create a subtask for every country.
        for country in res.countries.into_iter().take(10) {
            let name =
                "subtask_".to_string() + &remove_whitespace(&country.name);

            let params =
                subtask::SubtaskParams::new(country.name, country.href);
...
pub fn start() {
    let params =
        env::load_params::<MasterTaskParams>("static_content");
    let connector = processor::start();
